...
SET @MIN_AGE = 7 --Statistic must be older than x days
SET @MIN_ROWCOUNT = 10000 --The table must be have at least x rows
 
 
SELECT SCH.name AS SchemeName
   ,OBJ.name AS TableName
   ,STA.name AS StatisticName
   ,COALESCE(CONVERT(VARCHAR(25), STATS_DATE(STA.object_id, STA.stats_id), 120), 'never') AS LastStatisticUpdate
   ,TABLEROWCOUNT.TableRowCount
   ,DATEDIFF(d, ISNULL(STATS_DATE(STA.object_id, STA.stats_id), { D N'1900-01-01' }), GETDATE()) AS LastStatisticUpdateXDays
   ,'update　STATISTICS ['+SCH.name+'].['+OBJ.name+'] ('+STA.name+')' as updateSql
